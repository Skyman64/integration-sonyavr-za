...
async def on_connect_cmd() -> None:
    ...
    for receiver in _configured_devices.values():
        ...
        if receiver.available:
            _LOG.debug("Connect : device %s already active", receiver.address)
        ...
        if device_id in _configured_devices:
            ...
            if isinstance(entity, media_player.SonyMediaPlayer):
                api.configured_entities.update_attributes(
                    entity_id,
                    filter_attributes(device.attributes_for_zone(entity.zone), ucapi.media_player.Attributes),
                )
...
async def on_device_disconnected(avr_id: str):
    ...
    for configured_entity in _get_entities(avr_id):
        if configured_entity.entity_type == ucapi.EntityTypes.MEDIA_PLAYER:
            api.configured_entities.update_attributes(
                configured_entity.id,
...
def _register_available_entities(device_config: config.DeviceInstance, device: avr.SonyDevice) -> None:
    ...
    """
    entities = [
        media_player.SonyMediaPlayer(device_config, device, media_player.Zone.MAIN),
        media_player.SonyMediaPlayer(device_config, device, media_player.Zone.ZONE2),
        media_player.SonyMediaPlayer(device_config, device, media_player.Zone.ZONE3),
        ...
        sensor.SonySensorTuner(device_config, device),
    ]
...
async def main():
    ...
    for receiver in _configured_devices.values():
        if receiver.available:
            _LOG.debug("Main driver : device %s already active", receiver.address)
            continue
        await receiver.connect()
