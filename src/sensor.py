...
class SonySensorTuner(SonySensor):
    """Current tuner band/frequency/preset sensor entity."""

    ENTITY_NAME = "sensor_tuner"

    def __init__(self, device_config: DeviceInstance, device: avr.SonyDevice):
        """Initialize the class."""
        entity_id = f"{create_entity_id(device_config.id, EntityTypes.SENSOR)}.{SonySensorTuner.ENTITY_NAME}"
        self._device = device
        self._config_device = device_config
        super().__init__(entity_id, {"en": "Tuner", "fr": "Tuner"}, device_config, device)
    ...
    def update_attributes(self, update: dict[str, Any] | None = None) -> dict[str, Any] | None:
        ...
        if update:
            if ucapi.media_player.Attributes.STATE in update:
                attributes[Attributes.STATE] = SENSOR_STATE_MAPPING.get(update[ucapi.media_player.Attributes.STATE])
            if SonySensors.SENSOR_TUNER in update:
                attributes[Attributes.VALUE] = update[SonySensors.SENSOR_TUNER]
            return attributes
        return {
            Attributes.VALUE: str(self._device.receiver.tuner),
            Attributes.STATE: SENSOR_STATE_MAPPING.get(self._device.state),
        }
