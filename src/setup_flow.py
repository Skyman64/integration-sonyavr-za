    ...
    [
        ...
        },
        {
            ...
            "field": {
                "label": {
                    "value": {
                        "en": "Enter the receiver's IP address (e.g. 192.168.1.100). Auto-discovery is not supported.",
...
async def _handle_discovery(msg: UserDataResponse) -> RequestUserInput | SetupError:
    ...
    if address:
        ...
        try:
            ...
            # connection check via the receiver's web endpoint
            device_info = await config.Devices.extract_device_info(address)
            dropdown_items.append(
                {
                    "id": address,
                    "label": {"en": f"{device_info.name} [{address}]"},
                }
            )
        ...
        _LOG.warning("No address entered: auto-discovery is not supported for ES/ZA receivers")
